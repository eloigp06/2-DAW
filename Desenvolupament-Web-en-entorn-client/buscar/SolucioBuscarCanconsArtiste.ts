...
function songsSearch(artista:string, cancons:Canco[]): Canco[]{
    return cancons.filter(
        (c: Canco) => { return artista == c.artista}
    );
}
...
function songsPlayList(artista: string, playList: PlayList[]): Canco[] {
    ...
    for (let i: number = 0; i < playList.length; i++) {
        let songsArtista:Canco[] = songsSearch(artista, playList[i].cancons);
        cancons = cancons.concat(songsArtista);
    }
    return cancons;
}

function canconsArtistas(artista: string, username: string, users: User[]): Canco[] {
...
const cancons: Canco[] = canconsArtistas(artista, username, users)
console.log(cancons);
export { }